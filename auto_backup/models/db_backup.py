...
from ..mega.megatools import Mega
...
class DbBackup(models.Model):
    ...
    save_on_mega = fields.Boolean('Save file on MEGA?')

    mega_username = fields.Char('MEGA username')

    mega_pass = fields.Char('MEGA Password')

    mega_path = fields.Char('MEGA path')
    ...
    def action_backup(self):
        ...
        # We need to check if the backup is created and save this on MEGA
        if rec.save_on_mega and backup and filename:
            mg = Mega(mega_username=self.mega_username, mega_pass=self.mega_pass)
            pathexist = mg.ls(self.mega_path)
            # If the path doesn't exist on mega, we neeed to create this
            if not pathexist:
                path_split = self.mega_path.split('/')
                created_path = '/'
                for path in path_split:
                    if path:
                        aux_path = created_path + path
                        pathexist = mg.ls(aux_path)
                        if not pathexist:
                            mg.md(aux_path)
                        created_path = aux_path + '/'

            res = mg.put(rec.folder + '/' + filename, self.mega_path)
            # If the res has an error, we are trying again
            if res and res.get('error'):
                mg.put(rec.folder + '/' + filename, self.mega_path)
    ...
    def cleanup(self):
        ...
        for rec in self.filtered("days_to_keep"):
            with rec.cleanup_log():
                ...
                if rec.method == "local":
                    for name in iglob(
                        ...
                        if os.path.basename(name) < oldest:
                            os.unlink(name)
                    mega_file_name = os.path.basename(name)
                elif rec.method == "sftp":
                    with rec.sftp_connection() as remote:
                ...
                # If we have a copy on MEGA, we need to delete this
                if rec.save_on_mega:
                    mg = Mega(mega_username=rec.mega_username, mega_pass=rec.mega_pass)
                    mega_path = rec.mega_path if rec.mega_path[-1] != '/' else rec.mega_path[:-1]
                    fileexist = mg.ls(mega_path + '/' + mega_file_name)
                    if fileexist:
                        res = mg.rm(fileexist[0])
                        if res and res.get('error'):
                            mg.rm(fileexist[0])
